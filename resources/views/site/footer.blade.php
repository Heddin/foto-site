<?php
/**
 * Created by Alexander Pshenisnuk aka Heddin.
 * @mail: temm.twilightwizard@gmsail.com
 * @github: https://github.com/Heddin
 *
 * Created with: PhpStorm
 *
 * Date: 18.11.2017 * Time: 17:14
 */
?>
<footer class="page-footer grey darken-4 z-depth-1-half">
        <div class="container">
            <div class="row">
                <div class="col l10 s12">
                    <h5 class="white-text">Footer Content</h5>
                    <p class="grey-text text-lighten-4">You can use rows and columns here to organize your footer content.</p>
                </div>
                <div class="col l2 s12 right">
                    <h5 class="white-text">{{__('site.links')}}</h5>
                    <ul>
                        <li>
                            <a class="grey-text social_link" href="https://www.instagram.com/fotobykato">
                                <i class="fa fa-instagram"></i>
                                <span style="margin-left:5px">Instagram</span>
                            </a>
                        </li>
                        <li>
                            <a class="grey-text  social_link" href="https://www.facebook.com/photobykato">
                                <i class="fa fa-facebook"></i>
                                <span style="margin-left: 9px">Facebook</span>                            </a>
                        </li>
                        <li>
                            <a class="grey-text  social_link" href="https://vk.com/id140692989">
                                <i class="fa fa-vk"></i>
                                <span style="margin-left: 3px">Vkontakte</span>
                            </a>
                        </li>
                    </ul>
                </div>
            </div>
        </div>
        <div class="footer-copyright">
            <div class="container">
                © {{\Carbon\Carbon::now()->year}} fotobykato@gmail.com. {{__('site.rr')}}.
            </div>
        </div>
</footer>